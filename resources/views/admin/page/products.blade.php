...
@section('titleadmin', 'Quản lý sản phẩm')
    ...
    <div class="list-product">
        <table>
            ...
            @foreach ($products as $item)
                <tr>
                    <td class="product-checkbox">{{ $loop->iteration }}</td>
                    <td class="product-image"><img src="{{ asset('site/img/gallery/' . $item->images->first()->image) }}"
                            alt="">
                    </td>
                    <td class="product-name">{{ $item->name }}</td>
                    <td class="product-price">{{ number_format($item->price, 0, '.', ',') }} VNĐ</td>
                    <td class="product-price">{{ number_format($item->price_sale, 0, '.', ',') }} VNĐ</td>
                    <td class="product-category">{{ $item->categories->name }}</td>
                    <td class="product-category">{{ $item->brands->name }}</td>
                    <td class="product-category">{{ $item->created_at }}</td>
                    <td class="product-action">
                        <div class="edit-button">
                            <a href="{{ route('admin.product.edit', $item) }}" class="a">
                        ...
                        <a class="remove-button" href="{{ route('admin.product.delete', $item) }}">
